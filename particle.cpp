...
void Particle::AddParticleType(const char *name, const double mass, const int charge, double width = 0.)
{
    ...
    if (numParticleType_ < maxNumParticleType_)
    {
        if (width = 0.)
        {
            ...
            particleTypeArray_[numParticleType_] = new ParticleType{name, mass, charge};
        }
        else
        {
            particleTypeArray_[numParticleType_] = new ResonanceType{name, mass, charge, width};
        }
        ++numParticleType_;
    }
    else
        throw std::runtime_error("Exceeded max number of particle types. Please increase Particle::maxNumParticleType_");
}
...
double Particle::InvMass(Particle &p2)
{
    ...
    double sumPsquare = std::pow(pX_ + p2.pX_, 2) + std::pow(pY_ + p2.pY_, 2) + std::pow(pZ_ + p2.pZ_, 2);
